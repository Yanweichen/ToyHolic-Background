...
public class RedisClient {
	...
	public Object get(RedisKeyPrefix keyPrefix, String key) {
		...
		try {
			...
			LogTemplate.warn(se,"redisWarn","Error happened when getting data from Redis!");
			return null;
		}
	}
	...
	/**
	 * 计数用的方法。以给定的Key在redis中的值与输入的incr值相加，并存入redis；若给定的Key在redis中不存在，则直接保存incr的值到redis中。
	 * 此方法将重新设置给定Key在redis中的过期时间。
	 * 
	 * @param keyPrefix
	 * @param key
	 * @param incr 新增长的值，可以为负数；
	 * @param validPeriod 修改值以后redis中对应key的过期时间，单位是秒。
	 * @return 返回累加之后的值。
	 */
	...
	/**
	 * delete:方法描述 删除缓存的方法 直接根据输入的key做缓存删除
	 * 
	 * @author Leo
	 * @param key
	 *            缓存的key
